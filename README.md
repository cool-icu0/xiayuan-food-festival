...
```
陈连希项目整体架构设计，单体改微服务设计  占比19%
许纪超负责分类管理模块，单元测试         占比17%
王鹏负责菜品管理模块，单元测试           占比16%
关浩宇负责套餐管理模块，单元测试         占比16%
张佳红负责微信程序模块，数据库设计       占比16%
俞子琰负责订单明细模块，单元测试         占比16%
```
...
```
    Springboot
    MybatisPlus
    SpringCloud Alibaba（ Nacos、GateWay、OpenFeign）
    SpringCache+Redis
    SpringTask
    JWT
    WebSocket
